...
def load_geometry(params,mesh,mesh_facets,meshqual,rank,size,comm,remeshingm): ####################
   if remeshingm: # From gmsh #
      ...
      if not params.isaxis:
         ...
         create_gmshmesh_axis(params,meshfile,rank,size,comm)
...
def create_gmshmesh(params,meshfile,rank,size,comm):
   ...
   fpaux = 6.001#*random.uniform(0.9,1.1)
   lcar, lcor, lbod, lbex = params.lcar, params.lcor, params.lbod, params.lbod*2 #* (random.uniform(0.85,1.))
   gmsh.initialize()
   if rank == 0:
      ...
      for ig in range(params.nbg):
         for ib in range(params.nballg[ig]):
            ...
            if params.swmtype == 'circle':
               ...
               # Auxiliary points for mesh size near body's surface
               #pxmin = gmsh.model.geo.addPoint(xc[0]-radii*(1+fpaux),xc[1],0,lbex)
               #pxmax = gmsh.model.geo.addPoint(xc[0]+radii*(1+fpaux),xc[1],0,lbex)
               pymin = gmsh.model.geo.addPoint(xc[0],xc[1]-radih*fpaux,0,lbex)
               pymax = gmsh.model.geo.addPoint(xc[0],xc[1]+radih*fpaux,0,lbex)
               #gmsh.model.geo.rotate([(0,pxmin),(0,pxmax),(0,pymin),(0,pymax)],xc[0],xc[1],xc[2],axis[0],axis[1],axis[2],angle)
               gmsh.model.geo.rotate([(0,pymin),(0,pymax)],xc[0],xc[1],xc[2],axis[0],axis[1],axis[2],angle)
               pauxtags = pauxtags + [pxmin,pxmax,pymin,pymax]
      ...
      # Synchronize for physical groups
      gmsh.model.geo.synchronize()
      
      # Add auxiliary points in surface for mesh size near body's surface
      if True: gmsh.model.mesh.embed(0, pauxtags, params.nsd, fsurf)
...
def create_gmshmesh_axis(params,meshfile,rank,size,comm):
   random.seed(params.it)
   xmin, ymin = params.xmin, params.ymin
   Lx, Ly = params.Lx, params.Ly
   lcar, lbod, laxi = params.lcar, params.lbod, params.laxi #* (random.uniform(0.85,1.))
   gmsh.initialize()
   if rank == 0:
      gmsh.model.add("gmsh model")
      
      # Boundary box #
      pb0 = gmsh.model.geo.addPoint(xmin,   ymin,   0,lcar)
      pb1 = gmsh.model.geo.addPoint(xmin+Lx,ymin,   0,lcar)
      pb2 = gmsh.model.geo.addPoint(xmin+Lx,ymin+Ly,0,lcar)
      pb3 = gmsh.model.geo.addPoint(xmin,   ymin+Ly,0,lcar)
      #lb0 = gmsh.model.geo.addLine(pb0,pb1)
      lb1 = gmsh.model.geo.addLine(pb1,pb2)
      lb2 = gmsh.model.geo.addLine(pb2,pb3)
      lb3 = gmsh.model.geo.addLine(pb3,pb0)
      ...
      for ig in range(params.nbg):
         for ib in range(params.nballg[ig]):
            ...
            if params.swmtype == 'circle':
               ...
               pxmin = gmsh.model.geo.addPoint(xc[0]-radii,0,0,laxi)
               pxmax = gmsh.model.geo.addPoint(xc[0]+radii,0,0,laxi)
               #pymin = gmsh.model.geo.addPoint(xc[0],      0,0,lbod)
               pymax = gmsh.model.geo.addPoint(xc[0],radih  ,0,lbod)
               pcent = gmsh.model.geo.addPoint(xc[0],0,      0,lbod)
               #angle, axis = angle_axis_from_rotation(params.Qr[ig][ib],params.nsd)
               #gmsh.model.geo.rotate([(0,pxmin),(0,pxmax),(0,pymin),(0,pymax)],xc[0],xc[1],xc[2],axis[0],axis[1],axis[2],angle)
               ea1 = gmsh.model.geo.addEllipseArc(pxmin,pcent,pxmax,pymax)
               ea2 = gmsh.model.geo.addEllipseArc(pymax,pcent,pxmin,pxmax)
               #ea3 = gmsh.model.geo.addEllipseArc(pxmin,pcent,pxmax,pymin)
               #ea4 = gmsh.model.geo.addEllipseArc(pymin,pcent,pymax,pxmax)
               #balls.append(gmsh.model.geo.addCurveLoop([ea1,ea2,ea3,ea4]))
               btags.append([ea1,ea2])
               lbl = gmsh.model.geo.addLine(pb0,pxmin)
               axil.append(lbl)
               llbl.append(lbl)
               llbl.append(ea1)
               llbl.append(ea2)
      ...
      # Fluid domain #
      lbl = gmsh.model.geo.addLine(pb0,pb1)
      axil.append(lbl)
      llbl.append(lbl)      
      llb = gmsh.model.geo.addCurveLoop(llbl)
      fsurf = gmsh.model.geo.addPlaneSurface([llb])
      
      # Physical groups #
      gmsh.model.geo.synchronize()
      ptag = 1
      gmsh.model.addPhysicalGroup(1, axil, ptag)
      gmsh.model.setPhysicalName(1, ptag, "dwall")
      ptag += 1
      gmsh.model.addPhysicalGroup(1, [lb1], ptag)
      gmsh.model.setPhysicalName(1, ptag, "rwall")
...
def SolveLinearElasticityProblem(params,mesh,mesh_facets):
   ...
   if params.isaxis:
      bcsd = DirichletBC(W.sub(1), Constant(0), mesh_facets, 1) #down
   bcs.append(bcsd)
